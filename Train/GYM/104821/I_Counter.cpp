...
| URL:     https://codeforces.com/gym/104821/problem/I
| When:    2023-11-29 16:14:54
| 
| Memory:  1024 MB
| Time:    1000 ms
*******************************/
...
void solve() {
    ...
    sort(a.begin(), a.end());
    if(m == 1) {
        if(a[0].first >= a[0].second) cout << "Yes" <<endl;
        else cout << "No" <<endl;
        return;
    }
    int l;int r;
    for(int i = 0; i < m; ++ i) {
        ...
        int l=a[i].first-a[i-1].first-1;
        int r=a[i-1].second+l+1;
        if(i==0)continue;
        if((a[i].second>=0&&a[i].second<=l)||a[i].second==r);
        else ans=0;
