...
vector<C2> tr(N);
void dfs(int u) {
    if(tr[u].l == 0 && tr[u].r == 0) tans = min(ans, tans);
    if(tr[u].c == 'R') {
        if(tr[u].r != 0) dfs(tr[u].r);
        if(tr[u].l != 0) {
            ++ ans;
            dfs(tr[u].l);
            -- ans;
...
void solve() {
    flag = 0, U = 0, R = 0, L = 0, tans = INF, ans = 0;
    int n;
    string s;
    cin>> n >> s;
    for(int i = 1; i <= n; ++ i) {
