...
void solve() {
    ...
    bool flag = 0;
    if(k >= *min_element(a.begin() + 1, a.end())) flag = 1;
    for(int i = 1, j = 2; j <= n; ++ j) {
        if(h[j - 1] % h[j]) {
            if(i != j - 1) lr.push_back({i, j - 1});
            i = j;
        }else if(j == n) lr.push_back({i, n});
    }
    for(auto mm:lr) {
        for(ll i = mm.first, j = mm.first; i <= mm.second && j <= mm.second;) {
            if(pre[j] - pre[i - 1] <= k) ans = max(ans, j - i + 1), ++ j;
            else ans = max(ans, j - (i ++));
        }
    }
    cout<< (flag ? max(1ll, ans) : ans) <<endl;
}
signed main() {
