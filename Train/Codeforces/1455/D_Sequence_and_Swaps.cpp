...
void solve() {
    ...
    for(int i = 0; i < n; ++ i) {
        if(a[i] != na[i]) {
            ...
            if(i != 0 && a[i - 1] > a[i]) flag = 0;
        }
        if(i != 0) for(int j = i; j < n; ++ j) if(a[j - 1] > a[j]) break;
    }
    sort(sa.begin(), sa.end());
    if(sa == unsa) cout<< 0 <<endl;
    else cout<< (flag ? ans : -1) <<endl;
}
signed main() {
